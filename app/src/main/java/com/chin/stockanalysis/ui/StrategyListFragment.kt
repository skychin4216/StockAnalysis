...
class StrategyListFragment : Fragment() {
    ...
    private fun buildUI() {
        ...
        tuneBtn = Button(requireContext()).apply { text = "拟合"; textSize = 11f; setTextColor(Color.WHITE); setBackgroundColor(Color.parseColor("#EF6C00")); setPadding(6,6,6,6); setMinWidth(0); setMinimumWidth(0); layoutParams = LayoutParams(0,60,0.9f).apply { marginEnd = 3 }; setOnClickListener { runSelfTune() } }; row2.addView(tuneBtn)
        ...
        val importBtn = Button(requireContext()).apply { text = "导入"; textSize = 11f; setTextColor(Color.WHITE); setBackgroundColor(Color.parseColor("#2E7D32")); setPadding(6,6,6,6); setMinWidth(0); setMinimumWidth(0); layoutParams = LayoutParams(0,60,0.9f).apply { marginEnd = 3 }; setOnClickListener { importHistoricalData() } }; row2.addView(importBtn)
    ...
    private fun buildMemoryDialogContent(
        ...
        if (allSectors.isEmpty()) {
            ...
            for (sector in allSectors) {
                val chip = buildSectorChip(ctx, memory, sector.sectorName, sector.isActive)
        ...
        container.addView(android.widget.Button(ctx).apply {
            ...
            setOnClickListener {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    val suggested = (hot + news).distinct()
                        .filter { n -> updated.none { it.sectorName == n } }.take(8)
                    withContext(Dispatchers.Main) {
    ...
    private fun buildSectorChip(
        ...
        return android.widget.TextView(ctx).apply {
            ...
            textSize = 11f
            setPadding(20, 8, 20, 8)
            setTextColor(if (isActive) android.graphics.Color.WHITE else android.graphics.Color.parseColor("#999999"))
            background = android.graphics.drawable.GradientDrawable().apply {
                shape = android.graphics.drawable.GradientDrawable.RECTANGLE
                cornerRadius = 16f
                setColor(if (isActive) android.graphics.Color.parseColor("#4CAF50") else android.graphics.Color.parseColor("#EEEEEE"))
            }
            setOnClickListener {
