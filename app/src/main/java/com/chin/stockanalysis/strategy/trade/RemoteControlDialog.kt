 ...
 *
 * 通过局域网连接 PC 端 AutoQuant-GUI.exe(data_service.py + remote_control.py)：
 * - 提交远程任务：每日选股 / 滚动回测 / 自动拟合 / 刷新缓存 / 云上传 / 触发APK刷新
 * - 查看任务列表与状态（queued→running→success/failed/cancelled）
 * - 实时轮询任务日志
 * - 取消运行中的任务
 *
 * 依赖：PC 端已启动 data_service(默认 :8888)；Token 在 PC 端 AutoQuant/data/remote_token.txt。
 */
class RemoteControlDialog(context: Context) : Dialog(context) {
    ...
    private var host = PcBridgeClient.loadHost(context)
    private var token = PcBridgeClient.loadToken(context)
    ...
    private fun buildView(): View {
        ...
        root.addView(LinearLayout(context).apply {
            ...
            addView(TextBtn("🔄") { refreshTasks() })
            addView(TextBtn("✕") { dismiss() })
        })

        // ── 连接配置 ──
        val cfg = LinearLayout(context).apply {
            orientation = LinearLayout.VERTICAL
        ...
        cfg.addView(hostInput, ViewGroup.LayoutParams(MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT))
        cfg.addView(tokenInput, ViewGroup.LayoutParams(MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT))
        cfg.addView(TextView(context).apply {
            text = "🎛 远程控制 | 🔗 保存并测试"
            textSize = 13f
            setTextColor(Color.WHITE)
        ...
        msgPanel.addView(msgScroll, LinearLayout.LayoutParams(MATCH_PARENT, 150.dp()))
    ...
    private fun saveAndTest() {
        host = hostInput?.text?.toString()?.trim() ?: host
        token = tokenInput?.text?.toString()?.trim() ?: token
        PcBridgeClient.saveHost(context, host)
        PcBridgeClient.saveToken(context, token)
        statusTv?.text = "测试连接 $host …"
        scope.launch {
            try {
                val s = PcBridgeClient.remoteStatus(host, token)
                val j = JSONObject(s)
    ...
    private fun sendToCodeBuddy() {
        ...
        scope.launch {
            try {
                val r = PcBridgeClient.sendMsg(host, token, content)
                val j = JSONObject(r)
    ...
    private fun startMsgPolling() {
        ...
        msgPollJob = scope.launch {
            while (isActive) {
                try {
                    val r = PcBridgeClient.fetchMsgReplies(host, token, lastReplySeq)
                    val j = JSONObject(r)
    ...
    private fun submit(taskType: String) {
        ...
        scope.launch {
            try {
                val r = PcBridgeClient.submitTask(host, token, taskType)
                val j = JSONObject(r)
                if (j.optBoolean("ok", false)) {
    ...
    private fun refreshTasks() {
        ...
        scope.launch {
            try {
                val r = PcBridgeClient.listTasks(host, token)
                ...
                statusTv?.text = "共 ${tasks.length()} 个任务（最近） | PC: $host"
            } catch (e: Exception) {
    ...
    private fun taskCard(t: JSONObject): View {
        ...
        if (state == "running" || state == "queued") {
            ops.addView(TextView(context).apply {
                ...
                setOnClickListener {
                    scope.launch { PcBridgeClient.cancelTask(host, token, id); refreshTasks() }
                }
            }, LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT).apply {
                rightMargin = 6.dp()
            })
    ...
    private fun showLogs(taskId: String) {
        scope.launch {
            try {
                val r = PcBridgeClient.taskLogs(host, token, taskId, 0)
                val j = JSONObject(r)
    ...
    private fun startPolling(taskId: String) {
        ...
        pollJob = scope.launch {
            ...
            while (isActive) {
                try {
                    val r = PcBridgeClient.taskLogs(host, token, taskId, cursor)
                    val j = JSONObject(r)
                    val logs = j.optJSONArray("logs") ?: JSONArray()
                    for (i in 0 until logs.length()) {
                        val item = logs.getJSONObject(i)
