 ...
 */
class MaConvergenceNode : BaseNode<Any, MaConvergenceResult>("ma_convergence", "均線粘合偵測", NodeType.FACTOR_COMPUTE) {
    ...
    override suspend fun execute(context: PipelineContext, input: Any): MaConvergenceResult {
        return try {
            ...
            val hint = buildString {
                ...
                if (micro.oscillationHarvest) append("⚠️ 震蕩收割(${micro.oscillationCount}天高開低走) ")
                if (micro.volumeDivergence) append("⚠️ 量價背離(跌量>漲量) ")
