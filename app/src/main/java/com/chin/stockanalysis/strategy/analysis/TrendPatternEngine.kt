...
object TrendPatternEngine {
    ...
    /** 识别结果：cat/tag 与趋势图谱分类对齐；weight 为形态权重；stateLabel/stateBull 为 SAR 状态 */
    data class Match(
        val cat: String,
        val tag: String,
        val weight: Int,
    ...
    /**
     * 便捷：仅判断当前 SAR 状态是否偏多（供 AI 精选闸门等使用）
     */
