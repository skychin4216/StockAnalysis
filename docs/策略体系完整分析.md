...
### 12.2 评估结论：思路对，v2 已解决死值问题（v3 于 2026-09-18 落地，见 12.4）
...
RotationPenaltyNode v2（输入 MergedSignalPool + SectorHistoryData）
  A. 集中度惩罚（组合分散）—— ✅ 已实现（log2 对数衰减替代线性）：
     ...
     ✅ 龙头豁免 0.5 系数：2026-09-18 v3 已实现（板块内强度前 leaderExempt 只减半）
     ✗ 按"一级行业"统计（一票一板块）：未实现
  B. 轮动过热惩罚（时间维度）—— ◐ 部分实现：
  ...
  D. 参数按周期差异化 —— ✅ 2026-09-18 v3 已实现：短线(3/10/豁免2)、中线(5/15/豁免2)
```

### 12.4 v3 修复记录（2026-09-18，CodeBuddy 实现双端同构）

> 对应 12.2 遗留的 4 个实质差距，全部落地（Python `usecase_pipeline.py` + Kotlin
> `RotationPenaltyNode`/`SmartMoneyFilterNode`/`NodeRegistry` + 8 个周期 pipeline XML）：

| # | 遗留问题 | v3 修复 |
|---|----------|---------|
| 1 | 多板块重复累计放大惩罚（行业+概念各+1） | **一票一板块**：每票只投「当前命中数最少」的所属板块（按代码排序的确定性贪心，双端同规则） |
| 2 | 无龙头豁免 | **龙头豁免**：受罚板块内评分前 `leaderExempt`（默认 2，0=关闭）只惩罚减半，输出 `sectorLeaders` 由 smart_money_filter 消费（豁免按全部所属板块判定归属） |
| 3 | 参数名 `thresholdDays` 语义误导 | 改名 **`maxCountPerSector`**（兼容旧名读取），新参数 `leaderExempt` |
| 4 | 中线未差异化参数 | 中线 4 个 XML 显式配 **5/15/豁免2**（轮动周期长 → 阈值放宽、力度加重） |
| 5 | （parity bug 顺带修复）PC 端 smart_money_filter 惩罚取 sum 跨板块叠加 | 改为 **min（取最重惩罚）**，对齐 APK `minOrNull` 口径 |

实测（2026-09-18 午间 DAG）：半导体由 6 次/-34（重复累计+退潮标签）修正为 3 次/-5
（去重后仅 3 只 + hotDays=0 启动期 ×0.5）；光通信 -30 但龙头亨通光电/长飞光纤豁免减半；
15 只降分中 7 只豁免。仍**未实现**：板块近 N 日累计涨幅过热判定（需板块价格指数数据）、
generate_orders 同板块建仓数量限制、惩罚随时间衰减。
