# APK ↔ PC(exe/CodeBuddy) 免 IP 实时通讯：重新设计

> 目标：**像两个微信/QQ 一样，只要两端都能联网，就自动互通，不需要用户填 IP。**
> 状态：方案设计定稿（2026-09-12）。落地优先级：COS 中转 v1 → MQTT v2（按需）。
...
- PC 端：`AutoQuant/autoquant/data_service.py`
  - `DEFAULT_PORT = 8888`，`ThreadingHTTPServer((host, port))`，默认监听 `0.0.0.0`
  - 鉴权：请求头 `X-Token` 必须等于 `remote_control.get_token()`（存 `AutoQuant/data/remote_token.txt`）
...
- APK 端：`app/src/main/java/com/chin/stockanalysis/stock/data/PcBridgeClient.kt`
  ...
  - 业务方法（与 PC 路由一一对应）：
    `remoteStatus`、`taskTypes`、`submitTask`、`getTask`、`listTasks`、`cancelTask`、`taskLogs`、
    `sendMsg`、`fetchMsgReplies(after)`、`fetchCandidates`、`fetchRotation`、`fetchEtfLive`、`watch()` 轮询
- 预置配置：`app/src/main/assets/data/app_config.json` → `remote_control.preset_host = "192.168.1.3"`、`preset_port = 8888`、`preset_token`

### 1.2 为什么"必须联网就通"做不到

| 障碍 | 说明 |
|---|---|
| **PC 无公网地址** | 家庭/办公宽带在 NAT 后，没有公网 IP，APK 无法从外网直连 |
| **局域网寻址** | `192.168.x.x` 只在同一网段有效；换 WiFi、PC 重启换 IP 就得重填 |
| **NAT 穿透需要第三方** | 两端都在 NAT 后时，必须有一个「双方都能访问的中间人」 |
| **token 只是鉴权** | 有 token 也解决不了"找不到对方"的问题 |

### 1.3 微信/QQ 为什么能做到

它们**有一个永远在线的服务端**：
1. 账号体系 → 唯一的 `账号 ID` 取代 `IP`
2. 服务端中转 → 双方各自与服务器保持长连接（移动端 + 厂商推送保活）
3. 消息路由 → 服务器把消息投递给目标账号

**结论：只要想"免 IP"，就必须引入一个双方都可访问的中转。** 本项目**已经有这个中转的现成基础 = 腾讯云 COS**。
...
> 也就是说：**两端都已经能读写同一个 COS 桶**。把它当成"消息中转站"，就能立刻做到免 IP。
...
|---|---|---|---|---|---|
| **A. COS 中转（轮询）** | 2~10 s | **零** | ✅ 完全复用 | 选股/研报/任务这类分钟级交互 | ✅ **v1 首选** |
| **B. MQTT（公共/自建 broker）** | < 0.5 s | 需 broker | ❌ 需新依赖 | 需要秒级/实时推送 | ⭐ v2 升级 |
| **C. WebSocket 云托管/云函数** | < 0.3 s | 需常驻服务 | 部分 | 实时聊天式交互 | 备选 |
| **D. 内网穿透（frp/cpolar）** | < 0.1 s | 需公网服务器 + 常驻 | ❌ | 临时调试 | 不推荐 |
| **E. 企业微信机器人** | 秒级 | 零 | ✅ | **仅单向通知** | 只能做通知 |

**选型：A（立刻可用、零运维）+ 保留 B 作为升级路径。**
本项目的交互（选股、研报、回测、任务下发）本质是"**请求-响应**"，秒级延迟完全够用；真正需要 <1s 的只有"实时对话"，那一步再上 MQTT。
...
├── devices/
│   └── {deviceId}.json               # 设备注册/心跳：{name, lastSeen, mode, pubTokenHint}
...
- **按 `deviceId` 分目录**：多设备隔离，也不会读到别人的消息。
...
```json
{
  ...
  "ok": true,                          // resp 时有效
  "data": { },                         // resp 时有效（沿用现有 HTTP 返回体）
  "err": null,
  "sig": "hex(hmac_sha256(deviceToken, payload_without_sig))"
}
```

### 4.3 方法表（直接把现有 HTTP 路由平移为 method）
...
| 问题 | 处理 |
|---|---|
| 重复投递 | 以 `msgId` 为幂等键，两端各维护一份已处理 `msgId` 集合（PC 落盘 `bridge_state.json`，APK 用 SharedPreferences 环形缓存 500 条） |
...
| 重试 | APK 侧发消息失败 → 指数退避重试 3 次（1s/3s/9s）→ 仍失败则落本地待发表，联网后续传 |
...
| APK 前台且停留在相关页面 | **5 s** |
| APK 前台其他页面 | 15 s |
| APK 后台 | **不轮询**（用企业微信/系统通知唤醒，或用户打开时全量拉取） |
...
| 鉴权 | 每设备独立 `deviceToken`（32 字节随机），PC 端存 `bridge_devices.json`；旧 token 可一键吊销 |
| 完整性 | 消息体 `HMAC-SHA256(deviceToken, payload)` 签名，两端校验 `sig` |
| 密钥 | 优先用 **STS 临时密钥**（最小权限：仅该 `bridge/` 前缀的 Get/Put/List/Delete）；退路用现有 `secret_enc` 解出的主密钥 |
...
| `AutoQuant/autoquant/relay_worker.py` | **新增**：守护线程轮询 `pc/inbox/` → 调用现有 handler → 写 `pc/outbox/` |
| `AutoQuant/autoquant/data_service.py` | 抽出 `Transport` 抽象（`LocalHTTPTransport` / `CosRelayTransport`），路由 handler 复用；启动时按配置同时起 HTTP + relay worker |
| `AutoQuant/autoquant/data_service.py` | 推送落点增加：企业微信推送的同时写 `pc/push/{deviceId}/` |
| `AutoQuant/autoquant/remote_control.py` | `remote_token.txt` 之外增加 device 管理（注册/吊销/心跳超时） |
| `smalltools/cos_utils.py` | 抽 `sign/request/load_cloud_config` 为公共模块供 relay 复用（或直接 import） |

### APK
| 文件 | 改动 |
|---|---|
| `stock/data/CosRelayClient.kt` | **新增**：基于现有 `CloudSyncManager`/`CosSigner` 实现 `LIST/PUT/GET` + 信封编解码 + `sig` 签名 |
| `stock/data/PcBridgeClient.kt` | 抽出 `Transport` 接口（`send(method, params): JSONObject`），现有 HTTP 逻辑变 `LanTransport`，新增 `RelayTransport`；`watch()` 支持两种通道 |
| `stock/data/RemoteConfig.kt` | `remote_control` 增加 `mode: "auto" | "lan" | "relay"`、`relay_prefix`；默认 `auto` |
| `strategy/trade/RemoteControlPanel.kt` | 把"填 IP/端口/token"折叠进**高级选项**，默认展示"已通过云端中继连接" |
| `ui/`（设置页） | 显示中继连接状态、设备名、一键重新注册 |
| `assets/data/app_config.json` | `remote_control` 增补 `mode`、`relay_prefix` 默认值 |

### 迁移策略（不破坏现有用法）
`mode = "auto"` 的判定顺序：
1. 有局域网地址 → 直接 HTTP 探测 `/remote/status`（超时 **1.5 s**）
2. 成功 → 走局域网（延迟最低）
3. 失败或地址为空 → **自动切 COS 中继**，无需用户干预
4. UI 明确展示当前通道（局域网 / 云端中继）

> 结论：**用户从此不用再填 IP**；填 IP 变成"可选加速项"。
...
| 风险 | 说明 | 缓解 |
|---|---|---|
| 延迟 2~10 s | 轮询固有 | 选股/研报本就分钟级；要秒级上 MQTT |
| PC 必须在线 | 中转只是"找人"，执行仍在 PC | PC 侧已有 `_publish_candidates.py` 盘段守护/服务常驻 |
| COS 请求配额 | LIST 频率过高会被限流 | 5s 节奏 + 前缀 LIST + 必要时上 MQTT |
| 密钥泄漏 | `cloud_config.json` 里含 `secret_enc` | 确认其**不在 git 跟踪范围**；改用 STS 临时密钥 + 最小权限策略 |
| 离线补拉 | APK 后台不轮询期间的消息 | 结合企业微信通知唤醒；上线后按 `cursors` 全量补拉 |
| 单点依赖 | COS 故障则中继不可用 | 局域网直连仍保留为 fallback |
...
| 阶段 | 内容 | 产出 |
|---|---|---|
| **P0** | PC 端 `cos_relay.py` + `relay_worker.py`；APK 端 `CosRelayClient.kt`；跑通 `status.get` / `candidates.fetch` 两个只读方法 | 证明"不填 IP 也能取到选股结果" |
| **P1** | 全量方法表平移 + 幂等/游标 + `mode=auto` 自动降级 + UI 状态展示 | 日常完全不再需要 IP |
| **P2** | PC 主动推送落 `pc/push/`（与现有企业微信推送并联） | 选股完成自动到达 APK |
| **P3** | MQTT 替换运输层（信封/方法表不变） | 秒级双向 |
