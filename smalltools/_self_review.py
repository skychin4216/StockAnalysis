...
def load_cache_maps():
    ...
    进程内记忆（data/kline_store.json ≈135MB，单次 CLI/守护子进程内避免重复解析）。
    """
    mtime = None
    try:
...
# 输入只依赖 data/kline_store.json + COS phone 镜像，离线可用（三指数近似成长/权重风格：
...
IDX_NAME = {"sh000001": "上证", "sz399001": "深成", "sz399006": "创业板",
            "sh000688": "科创50"}  # sh000688 2026-09-06 起纳入 kline_store 池（腾讯日K，2021 起全史）
