...
| `_etf_buy.py` | **ETF 专买择时算法（2015→今 全历史回溯拟合, 2026-09-06 新增）**：拉 2015 前成立的 13 只宽基/行业 ETF 全历史 qfq 日K(腾讯 fqkline 翻页 640根/页) → 落盘 `_etf_cache.json`。信号体系(右侧突破/左侧低吸/底部反转/均线跟随) + **单仓状态机(同标的单笔)+信号冷却30天+大盘结构多头门控(沪深300 MA20>MA60)** → 两步网格拟合(①信号②离场) + IS(2015~2022)/OOS(2023~今)+时间切片 双段验证 → `data/_etf_fit_result.json`(含分标的明细)。v0.2 实测: dip_buy 低吸(回撤-12~-25%+RSI6<30+年线上方)×止盈1.5%/损-6%/30日 → FULL 90笔 胜率86.7% +175% / OOS 20笔 胜率90% 回撤-6.3%。诚实声明: 高胜率≠100%盈利, 无圣杯。 | `python _etf_buy.py` (自动断点续拉) / `--fetch-only` / `--codes sh510300,sz159915` / `--no-gate` 对照 |
