...
> 修改 exe 端文件要在 `AutoQuant/.git` 单独 commit；`smalltools/` 为双端共享 Python 引擎层（主仓管理）。
...
| ETF 低位(双端tab) | `skills/engine-sync` + exe `gui/etf_tab.py` + app `EtfDipFragment.kt` | `smalltools/_etf_buy.py v0.3 --live` → json → 双端同源；APK 走 PC 桥 `/etf_live` |
