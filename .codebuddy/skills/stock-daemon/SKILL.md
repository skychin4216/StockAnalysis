...
## ⚡ 执行纪律（2026-09-19 用户明确要求 —— 违反即返工）

用户原话：**"不要问这么频繁，我以为你已经实现完成了"**。据此定死：

**1. 绝不用「确认」代替「执行」。**
已被用户点名要求做的事（"继续做"、"要"、"实现完成后…"），**直接做完再汇报结果**。
禁止出现："要我继续吗"、"请确认先跑哪个"、"我建议 X，说一声我就启动"、"先确认一点…"。

**2. 长任务（>2 分钟）自己启动。**
回溯/拟合/回测这类长任务：**立刻用后台方式跑起来**，然后告诉用户「已启动 + 怎么看进度 + 预计多久」。
不要因为"怕跑错"就先问——跑错了可以重跑，问等才是真浪费。

**3. 技术卡点自己解决。**
接口不明 → 自己读源码；数据格式不符 → 自己安全适配（**保留原路径回退、默认行为不变**）；
已有能力已覆盖 → 直接复用，别再造一套。
只有在「不可逆破坏性操作」或「需求语义有歧义且代码无法推断」时才问，且**最多问一次**。

**4. 分多任务并行，别串着等。**
用户会明确要求并行（"一些任务回溯拟合，一些任务整理架构"）——
这时**同时起多个 subagent / 多个后台进程**，不要一个做完再开下一个。

**5. 汇报时说清"已完成什么"，而不是"我准备做什么"。**
结构：已完成的动作 + 实测结果（贴真实输出）+ 遗留项（如果有）。

> 这条纪律是对**所有** StockAnalysis 会话生效的默认行为，不只是本 skill。
